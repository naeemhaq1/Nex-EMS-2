...
import { executeHoursReport } from '../generate-comprehensive-hours-report';
import { executeVisualHoursReport } from '../generate-visual-hours-report';
import { generateAccurateHoursReport } from '../generate-accurate-hours-report.js';
...
// Generate comprehensive employee hours report
router.post('/generate-hours-report', requireAuth, requireAdmin, async (req, res) => {
  try {
    const { month, year } = req.body;
    const adminUser = req.session?.username || 'admin';

    console.log(`[Admin API] Generating comprehensive hours report requested by ${adminUser}${month && year ? ` for ${month}/${year}` : ' for current month'}`);

    // Execute the report generation (async)
    executeHoursReport(month, year)
      .then(() => {
        console.log('[Admin API] Hours report generation completed successfully');
      })
      .catch((error) => {
        console.error('[Admin API] Hours report generation failed:', error);
      });

    // Return immediate response
    res.json({ 
      success: true, 
      message: 'Hours report generation started. The report will be emailed when ready.',
      requestedBy: adminUser,
      period: month && year ? `${month}/${year}` : 'Current month'
    });

  } catch (error) {
    console.error('Error starting hours report generation:', error);
    res.status(500).json({ error: 'Failed to start hours report generation' });
  }
});

// Generate visual comprehensive employee hours report with charts, tables, and punctuality grading
router.post('/generate-visual-hours-report', requireAuth, requireAdmin, async (req, res) => {
  try {
    const { month, year } = req.body;
    const adminUser = req.session?.username || 'admin';

    console.log(`[Admin API] Generating comprehensive visual hours report with charts and tables requested by ${adminUser}${month && year ? ` for ${month}/${year}` : ' for current month'}`);

    // Execute the visual report generation (async)
    executeVisualHoursReport(month, year)
      .then(() => {
        console.log('[Admin API] Visual hours report generation completed successfully');
      })
      .catch((error) => {
        console.error('[Admin API] Visual hours report generation failed:', error);
      });

    // Return immediate response
    res.json({ 
      success: true, 
      message: 'Comprehensive visual hours report with charts, tables, and punctuality grading started. The enhanced report will be emailed when ready.',
      requestedBy: adminUser,
      period: month && year ? `${month}/${year}` : 'Current month',
      features: [
        'Complete employee data table with all hours worked and deductions',
        'Punctuality grading based on shifts (A+ to F scale)',
        'Interactive charts and performance analytics',
        'Strategic recommendations and action items',
        'Department performance analysis with visual insights'
      ]
    });

  } catch (error) {
    console.error('Error starting visual hours report generation:', error);
    res.status(500).json({ error: 'Failed to start visual hours report generation' });
  }
});

// Generate accurate hours report with correct business rules from raw attendance data
router.post('/generate-accurate-hours-report', requireAuth, requireAdmin, async (req, res) => {
  try {
    const { month, year } = req.body;
    const adminUser = req.session?.username || 'admin';

    console.log(`[Admin API] Generating ACCURATE hours report with corrected business rules requested by ${adminUser}${month && year ? ` for ${month}/${year}` : ' for July 2025'}`);

    // Execute the accurate report generation (async)
    generateAccurateHoursReport(month, year)
      .then((result) => {
        console.log('[Admin API] Accurate hours report generation completed successfully:', result);
      })
      .catch((error) => {
        console.error('[Admin API] Accurate hours report generation failed:', error);
      });

    // Return immediate response
    res.json({ 
      success: true, 
      message: 'ACCURATE hours report generation started with corrected business rules. The report will be emailed when ready.',
      requestedBy: adminUser,
      period: month && year ? `${month}/${year}` : 'July 2025',
      businessRules: [
        'Punch-in means present till punch-out, if no punch-out the time is capped at 7.5 hrs',
        'Complete days: Both check-in and check-out with actual time difference minus 1hr break',
        'Minimum 160 hours monthly threshold for salary deduction analysis',
        'Raw attendance data analysis (ignores pre-calculated summaries)',
        'Biometric-only employees included (excludes non-biometric exemptions)'
      ]
    });

  } catch (error) {
    console.error('Error starting accurate hours report generation:', error);
    res.status(500).json({ error: 'Failed to start accurate hours report generation' });
  }
});

// Complete July 2025 Monthly Report - ALL 309 biometric employees
router.get('/monthly-report/july-2025-full', requireAuth, requireAdmin, async (req, res) => {
  try {
    console.log('[Admin API] Fetching COMPLETE July 2025 report for ALL 309 biometric employees');

    // Get ALL biometric employees (excluding exemptions)
    const allEmployeesQuery = await db.execute(sql`
      SELECT DISTINCT 
        er.employee_code,
      ...
      ORDER BY er.employee_code
    `);

    console.log(`[Admin API] Found ${allEmployeesQuery.length} biometric employees to process`);

    // OPTIMIZED: Calculate ALL employee metrics with a single query
    const allEmployeeMetrics = await db.execute(sql`
      WITH employee_daily_summary AS (
        SELECT 
          er.employee_code,
          er.first_name,
          er.last_name,
          ...
          -- Calculate daily hours using CORRECT business rules
          CASE 
            WHEN COUNT(CASE WHEN ar.check_in IS NOT NULL THEN 1 END) > 0 
                 AND COUNT(CASE WHEN ar.check_out IS NOT NULL THEN 1 END) > 0 
                 AND MIN(CASE WHEN ar.check_in IS NOT NULL THEN ar.check_in WHEN ar.check_out IS NOT NULL THEN ar.check_out END) != 
                     MAX(CASE WHEN ar.check_in IS NOT NULL THEN ar.check_in WHEN ar.check_out IS NOT NULL THEN ar.check_out END) THEN
              -- Complete day: actual hours minus 1 hour break, minimum 6 hours
              GREATEST(
                EXTRACT(EPOCH FROM (
                  MAX(CASE WHEN ar.check_in IS NOT NULL THEN ar.check_in WHEN ar.check_out IS NOT NULL THEN ar.check_out END) - 
                  MIN(CASE WHEN ar.check_in IS NOT NULL THEN ar.check_in WHEN ar.check_out IS NOT NULL THEN ar.check_out END)
                )) / 3600.0 - 1.0,
                6.0
              )
            WHEN COUNT(CASE WHEN ar.check_in IS NOT NULL THEN 1 END) > 0 
                 AND COUNT(CASE WHEN ar.check_out IS NOT NULL THEN 1 END) = 0 THEN
              7.5  -- Punch-in only: capped at 7.5 hours
            WHEN COUNT(*) > 0 THEN
              7.5  -- Has attendance records: full day credit
            ELSE 0.0
          END as daily_hours,
          
          -- Track missed punch-outs
          CASE 
            WHEN COUNT(CASE WHEN ar.check_in IS NOT NULL THEN 1 END) > 0 
                 AND COUNT(CASE WHEN ar.check_out IS NOT NULL THEN 1 END) = 0 THEN 1
            ELSE 0
          END as missed_punchout
          
        FROM employee_records er
        LEFT JOIN attendance_records ar ON er.employee_code = ar.employee_code 
          AND DATE(ar.date) >= '2025-07-01'::date 
          AND DATE(ar.date) <= '2025-07-31'::date
        WHERE er.employee_code LIKE '10%'
          AND er.is_active = true
          AND er.employee_code NOT IN (
            SELECT employee_code 
            FROM biometric_exemptions 
        ...
        GROUP BY er.employee_code, er.first_name, er.last_name, er.department, er.position, DATE(ar.date)
      ),
      employee_monthly_summary AS (
        SELECT 
          employee_code,
          first_name,
          last_name,
          ...
          COUNT(CASE WHEN daily_hours > 0 THEN 1 END) as attendance_days,
          SUM(missed_punchout) as missed_punches
        FROM employee_daily_summary
        GROUP BY employee_code, first_name, last_name, department, position, employee_name
      )
      SELECT 
        employee_code,
        employee_name,
        ...
        COALESCE(attendance_days, 0) as attendance_days,
        COALESCE(missed_punches, 0) as missed_punches,
        CASE 
          WHEN attendance_days > 0 THEN ROUND((total_hours / attendance_days)::numeric, 1)
          ELSE 0
        END as avg_hours_per_day,
        CASE 
          WHEN attendance_days >= 22 THEN 'A+'
          WHEN attendance_days >= 20 THEN 'A'
          WHEN attendance_days >= 18 THEN 'B'
          WHEN attendance_days >= 15 THEN 'C'
          WHEN attendance_days >= 10 THEN 'D'
          ELSE 'F'
        END as punctuality_grade,
        CASE 
          WHEN attendance_days >= 22 THEN 95
          WHEN attendance_days >= 20 THEN 85
          WHEN attendance_days >= 18 THEN 75
          WHEN attendance_days >= 15 THEN 65
          WHEN attendance_days >= 10 THEN 55
          ELSE 0
        END as punctuality_score,
        CASE WHEN total_hours < 160 THEN true ELSE false END as is_min_hour_breach
      FROM employee_monthly_summary
      ORDER BY employee_code
    `);

    console.log(`[Admin API] OPTIMIZED query completed: ${allEmployeeMetrics.length} employees processed`);

    // Transform to required format
    const employeeData = allEmployeeMetrics.map(emp => {
      const comments = [];
      if (emp.missed_punches > 0) {
        comments.push(`${emp.missed_punches} missed punch-out(s)`);
      }
      if (emp.is_min_hour_breach) {
        comments.push(`Below 160h minimum (${parseFloat(emp.total_hours).toFixed(1)}h)`);
      }
      ...
      return {
        ...
        totalWorkedHours: Math.round(parseFloat(emp.total_hours || 0) * 10) / 10,
        attendanceDays: parseInt(emp.attendance_days || 0),
        averageHoursPerDay: parseFloat(emp.avg_hours_per_day || 0),
        punctualityGrade: emp.punctuality_grade,
        punctualityScore: parseInt(emp.punctuality_score || 0),
        missedPunches: parseInt(emp.missed_punches || 0),
        deductedHours: 0,
        adjustedHours: Math.round(parseFloat(emp.total_hours || 0) * 10) / 10,
        overtimeHours: Math.max(0, parseFloat(emp.total_hours || 0) - 160),
        isMinHourBreach: emp.is_min_hour_breach,
        comments: comments.join(', ') || 'Regular attendance'
      };
    });
    ...
    const summary = {
      ...
      totalBiometricEmployees: employeeData.length,
      employeesWithData: employeeData.filter(e => e.attendanceDays > 0).length,
      zeroHoursEmployees: employeeData.filter(e => e.totalWorkedHours === 0).length,
      minHourBreachCount: employeeData.filter(e => e.isMinHourBreach).length,
      topPerformerCount: employeeData.filter(e => e.totalWorkedHours >= 200).length
    };
    ...
    console.log(`[Admin API] Complete July 2025 report generated: ${employeeData.length} employees, ${summary.employeesWithData} with data`);
    res.json(response);

  } catch (error) {
    console.error('[Admin API] Error generating complete July 2025 report:', error);
    res.status(500).json({ 
      error: 'Failed to generate complete monthly report',
      details: error.message 
    });
  }
});
